...
function getMovieData(){
	var theatreData = fs.readFileSync('data.json', 'utf8');
...
app.post('/movies/:id', function(req, res){
	...
	request("http://www.omdbapi.com/?i="+movieID+"&r=json", function(err, response, body) {
            ...
            if(!err){
              ...
              var newId = parseInt(movieData.counter) + 1;
              var newMovie={
	 			"id": newId,
              ...
              movieData.movies[newId] = newMovie;
              movieData.counter = newId;
...
app.delete('/movies/:omdbid', function(req, res){
	var movieData = getMovieData();
	var moveToDelete = movieData.movies[req.params.omdbid];
	delete movieData.movies[req.params.omdbid];
	var newId = parseInt(movieData.counter) - 1;
	movieData.counter=newId;
	saveMovieData(movieData);
	res.redirect('/movies');
